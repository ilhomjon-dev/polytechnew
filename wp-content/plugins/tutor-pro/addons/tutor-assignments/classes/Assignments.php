...
class Assignments extends Tutor_Base {
	...
	public function change_assignment_single_url( $post_link, $id = 0 ) {
		...
		if ( is_object( $post ) && 'tutor_assignments' === $post->post_type ) {
			...
			if ( is_object( $course ) ) {
				return home_url( "/{$this->course_base_permalink}/{$course->post_name}/assignments/" . $post->post_name . '/' );
			} else {
				return home_url( "/{$this->course_base_permalink}/sample-course/assignments/" . $post->post_name . '/' );
			}
		}
	...
	public function tutor_assignment_submit() {
		...
		$assignment_id        = Input::post( 'assignment_id', 0, Input::TYPE_INT );
		$assignment_answer    = Input::post( 'tutor-global-text-editor', '', Input::TYPE_KSES_POST );
